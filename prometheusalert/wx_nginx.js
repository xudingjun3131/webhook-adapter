...
exports.template = function(body) {
    ...
    var content = alerts.map(
        ...
        .concat(`###### 容器区域:<font color="warning">${alert.labels.qys_area}</font>`)
        .concat(`###### 告警后端:<font color="comment">${alert.labels.upstream}</font>`)
        .concat(`###### 告警域名:<font color="comment">${alert.labels.filter}</font>`)
        .concat(`###### 告警状态码:<font color="comment">${alert.labels.filterName}</font>`)
